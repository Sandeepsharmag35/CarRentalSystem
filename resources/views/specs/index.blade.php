...
<div class="mx-8 text-left mt-20">
    <a href="{{route('specs.create')}}" class="bg-sidebar hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Add Specs</a>
</div>
    ...
    <div class="overflow-x">
        <table id="example" class="display w-auto">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{$spec->cars->name}}</td>
                    <td>{{$spec->bodyType}}</td>
                    <td>{{$spec->seatCapacity}}</td>
                    <td>{{$spec->doors}}</td>
                    <td><img src="{{asset('images/cars/'.$spec->cars->photopath)}}" alt="" class="w-20 h-20"></td>
                    

                    <td>
                        <div class="flex flex-wrap justify-center space-x-2 gap-2 sm:space-x-4 md:space-x-6">
                            <a href="{{route('specs.edit',$spec->id)}}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition duration-300 ease-in-out">Edit</a>
                            <a onclick="showDelete('{{$spec->id}}')" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg cursor-pointer transition duration-300 ease-in-out">Delete</a>
                        </div>
                    </td>
                </tr>
              @endforeach
...
<div id="deletebox" class=" hidden fixed inset-0 bg-blue-500 bg-opacity-40 backdrop-blur-sm">
    <div class="flex h-full justify-center items-center">
        <div class="bg-white p-10 ">
            ...
            <form action="{{route('specs.delete')}}" method="POST">
                @csrf
