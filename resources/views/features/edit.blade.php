...
<div class="container mx-auto mt-6 p-4">
    <h1 class="text-2xl font-semibold mb-4">Edit Features for a Car</h1>
    ...
    <form action="{{ route('features.update', $feature->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select name="car_id" id="car_id" class="w-full px-4 py-2 border rounded focus:ring focus:ring-indigo-300">
                <option value="{{ $feature->car_id }}" selected>{{ $feature->cars->name }}</option>
                @foreach($cars as $car)
    ...
    <textarea name="features[]" id="features" class="w-full px-4 py-2 border rounded focus:ring focus:ring-indigo-300" rows="4">{{ implode(', ', $feature->features) }}</textarea>
</div>



        <div class="mb-4">
            <button type="submit" class="bg-indigo-500 text-white px-4 py-2 rounded hover:bg-indigo-600 focus:outline-none focus:ring focus:ring-indigo-300">Update Features</button>
